...
public class XmlFileWriter : IFileWriter<string>
{
    ...
    private async Task WriteRawXmlAsync(string filePath, string xmlData, bool useIndent, string indentChars,
        bool expandEntities)
    {
        if (!xmlData.TrimStart().StartsWith("<?xml", StringComparison.Ordinal))
        {
        ...
        _exceptionHandler?.Handle(new InvalidOperationException("Used raw XML writing due to parsing issues"));
    }
