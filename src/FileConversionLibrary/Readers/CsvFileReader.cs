...
public class CsvFileReader : IFileReader<CsvData>
{
    ...
    private static async Task<char> AutoDetectDelimiterAsync(string filePath)
    {
    ...
    private async Task<CsvData> ReadInternalAsync(string filePath, char delimiter)
    {
        try
        {
            ...
            {
                throw new InvalidDataException("Failed to read CSV headers.");
            }
    ...
    private CsvData ReadWithCustomParser(string filePath, char delimiter)
    {
        try
        {
            ...
            _exceptionHandler?.Handle(new InvalidOperationException($"Used custom CSV parser for {filePath}"));
