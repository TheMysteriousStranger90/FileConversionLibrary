...
public class XmlFileReader : IFileReader<XmlData>
{
    ...
    private static string[] ExtractHeaders(XDocument doc)
    {
        if (doc.Root == null)
    ...
    private static List<string[]> ExtractRows(XDocument doc, string[] headers)
    {
        ...
        foreach (var element in rowElements)
        {
            ...
            foreach (var child in element.Elements())
            {
                ...
                if (repeatingChildren.Count > 0 && repeatingChildren.All(e => e.Name == repeatingChildren.First().Name))
                {
    ...
    private static string? FindRowElement(XDocument doc)
    {
    ...
    private async Task<XmlData> ManualParseXmlAsync(string filePath)
    {
        ...
        {
            _exceptionHandler?.Handle(new InvalidOperationException("Manual XML parsing failed", ex));
            throw;
        }
    }
}